...
func GetUserList(c *gin.Context) {
	...
	pkg.AssertErr(err, "抱歉未找到相关用户信息", -1)

	if len(result) > 0 {
		for i := 0; i < len(result); i++ {
			var conf models.UserLevel
			userLevel, err := conf.GetUserLevel(result[i].UserId)
			pkg.AssertErr(err, "未找到相关等级信息", -1)
			result[i].UserLevel = userLevel
		}
	}
...
func GetTeamList(c *gin.Context) {
	...
	param["pageSize"] = c.DefaultQuery("pageSize", "10")
	param["pageIndex"] = c.DefaultQuery("pageIndex", "1")
	result, err := data.GetTeamListByUserList()
	pkg.AssertErr(err, "抱歉未找到相关用户信息", -1)
...
// 一次性分配选择团队类型
func GetUserteamid(c *gin.Context) {
	var data models.SysUser
	//var res models.Response

	param := make(map[string]string)
	param["team_name"] = c.Request.FormValue("team_name")
	//param["pageSize"] = c.DefaultQuery("pageSize", "10")
	//param["pageIndex"] = c.DefaultQuery("pageIndex", "1")
	//param["deptid"] = c.Request.FormValue("dept_id")

	result, err := data.Getteam(param)
	//if err !=nil{
	//	res.Msg = err.Error()
	//	c.JSON(http.StatusOK, res.ReturnError(0))
	//}
...
//新加客户分润新增
//func GetUserpath(c *gin.Context) {
//	var data models.SysUser
//	//var res models.Response
//
//	param := make(map[string]string)
...
//	//param["deptid"] = c.Request.FormValue("dept_id")
//
//	result, err := data.GetSyspath(param)
//	//if err !=nil{
//	//	res.Msg = err.Error()
//	//	c.JSON(http.StatusOK, res.ReturnError(0))
//	//}
...
func Editlevellock(c *gin.Context) {
	...
	// 获取用户信息
	userInfo, err := sysUser.GetUserInfo(param["id"])
	if err != nil {
		res.Msg = err.Error()
		c.JSON(http.StatusOK, res.ReturnError(400))
		return
	...
	if userInfo.LevelLock == config.LevelLock && param["level_lock"] == config.LevelLock {
		res.Msg = "用户已锁定,勿重复锁定"
		c.JSON(http.StatusOK, res.ReturnError(400))
		return
	}
	...
	// 处理锁定相关
	if param["level_lock"] == config.LevelUnLock {
		// 关闭等级锁定
		err := sysUser.Closelevellock(param)
		if err != nil {
			res.Msg = err.Error()
		...
		// 打开等级锁定
		err := sysUser.Openlevellock(param)
		if err != nil {
			res.Msg = err.Error()
			c.JSON(http.StatusOK, res.ReturnError(500))
			return
	...
	// 录入锁定时候的数据信息
	str, _ := utils.StructToJsonStr(userInfo)
	loginlog.Msg += "  " + str
