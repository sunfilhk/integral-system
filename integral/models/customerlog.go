...
type CustomerLog struct {
	...
	ApplicationTime time.Time `gorm:"column:application_time" json:"application_time"` //创建时间
	Audittime       time.Time `gorm:"column:audittime" json:"audit_time"`              //审核时间
	InvestTime      time.Time `gorm:"column:invest_time" json:"invest_time"`           //投资时间
	Bank            string    `gorm:"column:bank" json:"bank"`
	Banknum         string    `gorm:"column:banknum" json:"banknum"`
	Edittype        int       `gorm:"column:edittype" json:"edittype"` //
	Customerid      string    `gorm:"column:customerid" json:"customerid"`
	...
	Tickname        string    `gorm:"column:tickname" json:"tick_name"`
...
func (e *CustomerLog) CustomerAuditList(param map[string]string) (result interface{}, err error) {
	...
	sql := ` select if(name=if(ifnull(namenew,'')='',name,namenew),name,CONCAT(name,' 改为 ',namenew))name,
		...
		investmentid,i.update_time application_time,i.invest_time,
		if(c.amount=if(ifnull(amountnew,'')='',c.amount,amountnew),c.amount,CONCAT(c.amount,' 改为 ',amountnew))amount, 
		c.create_time,c.update_time,c.status,u.nick_name nickname,c.id,u.team_name tickname ,u.team_id                                           
	from customer_log c
	left JOIN sys_user u on c.salesmanid = u.user_id
	left join investment i on c.investmentid = i.id
	where 1=1 ` + con

	//投资id
	web := param["investmentid"]
	if web != "" {
		sql += `and investmentid = '` + web + `'`
	}
	...
	if keyword != "" {
		sql += ` and (c.phone like '%` + keyword + `%' or c.name like '%` + keyword + `%' or investmentid like '%` + keyword + `%' ) ` //新加投资id查询
	}
	//总数
...
func (e *CustomerLog) CustomerLogList(param map[string]string) (result interface{}, err error) {
	...
	sql := ` select if(name=ifnull(namenew,name),name,CONCAT(name,' 改为 ',namenew))name,
		...
		c.create_time,c.update_time,c.update_time audittime,c.status,u.nick_name nickname,c.id,u.team_name tickname , u.team_id 
	from customer_log c
	left JOIN sys_user u on c.salesmanid = u.user_id
	where 1=1  ` + con
	//投资id
...
func (e *CustomerLog) CustomerLogAudit(param map[string]string) (err error) {
	...
	sql := ` update customer_log set status = :status,update_time=now() where id = :id and status <> :status` //修改时间
	...
	if param["status"] == "0" {
		...
		case "0":
			...
			sql3 := `update sys_user set accumulative = accumulative + real_value +` + utils.Float64ToString(in.Amount) + `,real_value=0 where user_id in (` + ref.Referrers + `) and level_lock=0`
		...
		case "4":
			...
			//减掉相关业务员业绩
			//备份分润信息
			sql2 := `select * from investmentprofit  where investmentid = ` + param["investmentid"]
			...
			//删除分润
			sql4 := `delete from investmentprofit where investmentid = :investmentid `
			sql4 = utils.SqlReplaceParames(sql4, param)
			if err = orm1.Exec(sql4).Error; err != nil {
				return
			}
			...
			//22/9/8/修改real_value减去到期业绩
			//sql3 := `update sys_user set real_value = real_value - ` + utils.Float64ToString(in.Amount) + ` where user_id in (` + ref.Referrers + `) and real_value>0  ` //22/9/9后面增加and real_value>0
			//if err = orm1.Exec(sql3).Error; err != nil {
			//	orm1.Rollback()
			//	return
			//}
