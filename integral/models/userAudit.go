...
type UserAudit struct {
	...
	TeamName string    `gorm:"column:teamname" json:"teamname"`   // 通过(拒绝)时间
}
...
func (this *UserAudit) GetAuditList(pageIndex int64, pageSize int64, curUserID, role string) (ret interface{}, total string, err error) {
	...
	sql := `select u1.*, u2.username as referrer_name,u2.team_name teamname from user_audit u1 left join sys_user u2 on u2.user_id = u1.referrer  ` //换u2.is_pass = u1.is_pass
	...
	sql += ` ORDER BY u1.pass_time DESC `
	sql += ` limit `
