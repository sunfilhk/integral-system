...
func (e *UserLevel) GetMaxAccumulative(id string) (float64, int, error) {
	sql := `select team_id,if(lifts<1,if(accumulative>set_level,if(level_lock=0,real_value+accumulative,GREATEST(set_level,lock_level)),GREATEST(set_level,lock_level)),GREATEST(set_level,lock_level)) accumulative from sys_user WHERE user_id = ` + id
	type Acc struct {
		Accumulative float64
		TeamId       int
	}
	...
	return acc.Accumulative, acc.TeamId, nil
}

// 专门给开启等级锁填充等级用
func (e *UserLevel) GetMaxAccumulativeLock(id string) (float64, int, error) {
	sql := `select team_id,if(lifts<1,if(accumulative>set_level,if(level_lock=0,real_value+accumulative,accumulative),set_level),set_level) accumulative from sys_user WHERE user_id = ` + id
	type Acc struct {
		Accumulative float64
		TeamId       int
	}
...
func (e *UserLevel) GetUserLevelLock(id string) (string, int64, error) {
	sql := `select level_lock,lock_level from sys_user WHERE user_id = ` + id
	...
	if len(acc.LevelLock) == 0 {
		acc.LevelLock = "0"
	}
	return acc.LevelLock, acc.LockLevel, nil
}
func (u *UserLevel) GetReferrerLevel(ids string, accumulative float64) ([]UserLevel, error) {
	refs := strings.Split(ids, ",")
	if len(refs) == 0 {
	...
	var ul []UserLevel
	for _, id := range refs {
		acc, ace, err := u.GetMaxAccumulative(id)
		if err != nil {
			return nil, err
		}
		astr := utils.Float64ToString(acc)
		sad := strconv.Itoa(ace)
		lock, _, err := u.GetUserLevelLock(id)
		if err != nil {
			return nil, err
		}
		...
		//			GROUP BY user_id order by levelvalue`
		sql := `select *,` + astr + ` accumulative,` + lock + ` level_lock from user_level 
					where levelvalue =
					(
...
func (u *UserLevel) GetSetUserByUserid(param map[string]string) (UserLevel, error) {
	userid := param["userid"]
	acc, ss, err := u.GetMaxAccumulative(userid)
	if err != nil {
	...
	sql := `select *,` + astr + ` accumulative from user_level 
         where team_id =` + ads + ` and levelvalue =
         (
            SELECT max(levelvalue) from user_level
            where  ` + astr + `  >= levelvalue and team_id= ` + ads + `   
         )`
	var ul UserLevel
	err = orm.Eloquent.Raw(sql).Scan(&ul).Error
	return ul, err
...
// 获取设置等级
func (u *UserLevel) GetSetUserLevel(levelvalue float64, id string) ([]UserLevel, error) {
	sql := `select * from user_level where levelvalue>=? and team_id = ? order by levelvalue `
	var ul []UserLevel
	err := orm.Eloquent.Raw(sql, levelvalue, id).Scan(&ul).Error
	return ul, err
}
...
func (this *UserLevel) EditUserVipLevel(userID string, levelID string) (err error) {
	sql := `select if(accumulative>set_level,accumulative,set_level) set_level,level_lock from sys_user where user_id = ` + userID
	...
	var sql2 string
	if setLevelVip[0].Levellock == "1" {
		sql2 = `update sys_user set set_level = ` + levelValueStr + `,lock_level=` + levelValueStr + `,lifts=` + nums + `,update_time = now() where user_id = ` + userID
	} else {
...
func (this *UserLevel) GetVipLevelList(userID string) (ret interface{}, err error) {
	...
	//	return
	//}
	accumulative, ss, err := this.GetMaxAccumulative(userID)
	if err != nil {
		return
	}
	...
	sql := `select CONCAT('V',(@rowNO := @rowNo+1)) AS vipLevel,a.* from  user_level a,(select @rowNO :=0) b where team_id =? ORDER BY levelvalue asc`
	vips := make([]vipLevelData, 0)
	if err = orm.Eloquent.Raw(sql, ss).Scan(&vips).Error; err != nil {
		return
	}
...
func (this *UserLevel) GetUserLevel(userID string) (ret string, err error) {
	accumulative, ss, err := this.GetMaxAccumulative(userID)
	if err != nil {
		return
	}
