...
type Config struct {
	...
	//Userid  int64 `json:"uesrid" gorm:"column:user_id;"`
	...
	Teamid   int64  `json:"teamid" gorm:"column:team_id"`     //新加字段
	Deptname string `json:"deptname" gorm:"column:dept_name"` //新加字段
	//Precent  float64 `json:"precentid" gorm:"column:precent_id"` //新加字段
	//Type       string `json:"type" gorm:"column:type"`             //变量类型:string,text,int,bool,array,datetime,date,file
...
//	sql := `select * from sys_config where configKey = ?`  //原来的sql语句
