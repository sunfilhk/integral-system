package models

import (
	"fmt"
	"strconv"
	"time"
	orm "xAdmin/database"
	"xAdmin/utils"
)

//部门
type Dept struct {
	//部门编码
	Deptid int64 `gorm:"column:deptId;primary_key" json:"deptId" example:"1" extensions:"x-description=标示"`

	//上级部门
	ParentId int64 `gorm:"column:parent_id" json:"parent_id"`

	DeptPath string `gorm:"column:dept_path" json:"deptath"`

	//部门名称
	Deptname string `gorm:"column:dept_name" json:"deptname"`

	//排序
	Sort int64 `gorm:"column:sort" json:"sort"`

	// 负责人ID
	LeaderID int64 `gorm:"column:leader_id" json:"leaderID"`

	//手机
	Phone string `gorm:"column:phone" json:"phone"`

	//邮箱
	Email string `gorm:"column:email" json:"email"`

	//状态
	Status string `gorm:"column:status" json:"status"`

	CreateBy string `json:"createBy" gorm:"column:create_by"`
	//CreateTime string `json:"createTime" gorm:"column:create_time"`
	CreateTime time.Time `json:"createTime" gorm:"column:create_time"`
	UpdateBy   string    `json:"updateBy" gorm:"column:update_by"`
	//UpdateTime string `json:"updateTime" gorm:"column:update_time"`
	UpdateTime time.Time `json:"updateTime" gorm:"column:update_time"`
	//DataScope  string `json:"dataScope" gorm:"_"`
	//Params     string `json:"params" gorm:"column:params"`
	IsDel string `json:"isDel" gorm:"column:is_del"`

	Children []Dept `json:"children"`
	//TeamId int64 `gorm:"column:team_id" json:"team_id"`
	//Profittype  int64 `gorm:"column:profit_type" json:"profit_type"`
	Profit float64 `gorm:"column:profit" json:"profit"`
	Isteam int     `gorm:"column:is_team" json:"isteam"`
}

type Once struct {
	Teamname string  `gorm:"column:team_name" json:"teamname"`
	Profit   float64 `gorm:"column:profit" json:"profit"`
	DeptId   int64   `gorm:"column:deptid" json:"deptid"`
	Status   int64   `gorm:"column:status" json:"status"`
}

type DeptInfo struct {
	Dept
	LeaderName string `json:"leader" gorm:"column:leader_name"` // 负责人名字, 不需要在部门表保存
}

type DeptLable struct {
	Id       int64       `json:"id" gorm:"-"`
	Label    string      `json:"label" gorm:"-"`
	Children []DeptLable `json:"children" gorm:"-"`
}

func (e *Dept) Create() (Dept, error) {
	var doc Dept
	e.UpdateTime = time.Now()
	e.CreateTime = time.Now()
	e.IsDel = "0"
	e.Profit = 0.05
	if e.ParentId == 0 {
		e.Isteam = 1
	} else {
		e.Isteam = 0
	}
	result := orm.Eloquent.Table("sys_dept").Create(&e)
	if result.Error != nil {
		err := result.Error
		return doc, err
	}
	deptPath := "/" + utils.Int64ToString(e.Deptid)
	if int(e.ParentId) != 0 {
		var deptP Dept
		orm.Eloquent.Table("sys_dept").Where("deptId = ?", e.ParentId).First(&deptP)
		deptPath = deptP.DeptPath + deptPath
	} else {
		deptPath = "/0" + deptPath
	}
	var mp = map[string]string{}
	mp["deptPath"] = deptPath
	if err := orm.Eloquent.Table("sys_dept").Where("deptId = ?", e.Deptid).Update(mp).Error; err != nil {
		err := result.Error
		return doc, err
	}
	doc = *e
	doc.DeptPath = deptPath
	return doc, nil
}

func (e *Dept) Profitcusto(param map[string]string) (result interface{}, err error) {
	//status := param["dept_name"]
	sql := `	select s.deptId,s.dept_name,s.profit,s.status,s.create_time,s.update_time from sys_dept as s where is_team = 1 `
	if param["deptId"] != "" {
		sql += `  and deptId = ` + param["deptId"]
	}
	//key := param["deptname"]
	//if key != "" {
	//	sql += ` and (dept_name like '%` + key + `%') `
	//}
	//orm.Eloquent.Raw(sql,param["param"]).Scan(&user)

	param["total"] = GetTotalCount(sql)
	//分页 and 排序
	param["sort"] = "deptId"
	param["order"] = "asc"
	sql += utils.LimitAndOrderBy(param)

	user := make([]Dept, 0)
	orm.Eloquent.Raw(sql).Scan(&user)

	result = user

	return

}

func (e *Dept) Gotonce(param map[string]string) (interface{}, error) {

	sql := `select s.deptId deptid,s.dept_name team_name,s.profit,s.status,s.create_time,s.update_time from sys_dept as s where is_team = 1` //加了dept_id和dept_name
	//keyword := param["dept_name"]
	if param["deptId"] != "" {
		sql += `  and deptId = ` + param["deptId"]
	}
	//param["total"] = GetTotalCount(sql)
	////分页 and 排序
	//param["sort"] = "user_id"
	//param["order"] = "desc"
	var w []Once
	//sql += utils.LimitAndOrderBy(param)
	err := orm.Eloquent.Raw(sql).Scan(&w).Error

	return w, err
}

func (e *Dept) Getteam(param string) (update Dept, err error) {
	//key:=param["profittype"]
	//key:=
	//sql := `select * from sys_config where team_id = ?`  //原来的sql语句
	fmt.Println("数据", param)
	sql := `SELECT s.*,u.user_id FROM sys_dept s LEFT JOIN sys_user u on s.dept_name=u.team_name WHERE	u.user_id= ?` //原来的sql语句
	err = orm.Eloquent.Raw(sql, param).Scan(&update).Error
	return
}

//修改客户分润
func (e *Dept) Updatecustomer(profit, deptId string) (err error) {

	//param["id"] = strconv.FormatInt(utils.Node().Generate().Int64(),10)

	sql := ` update sys_dept set profit =? where deptId=? `
	//sql = utils.SqlReplaceParames(sql,param)
	//if err = orm.Eloquent.Exec(sql).Error;err!=nil{
	//	return err
	//}
	//return
	err = orm.Eloquent.Exec(sql, profit, deptId).Error
	return
}

func (e *Dept) Agetcusto(param map[string]string) (interface{}, error) {
	sql := `select *from sys_dept where deptId in (1,14,17,18)` //加了dept_id和dept_name
	keyword := param["dept_name"]
	if keyword != "" {
		sql += ` and (dept_name like '%` + keyword + `%') `
	}
	//param["total"] = GetTotalCount(sql)
	////分页 and 排序
	//param["sort"] = "user_id"
	//param["order"] = "desc"
	var u []Dept
	//sql += utils.LimitAndOrderBy(param)
	err := orm.Eloquent.Raw(sql).Scan(&u).Error

	return u, err
}

func (e *Dept) Get() (Dept, error) {
	var doc Dept

	table := orm.Eloquent.Table("sys_dept")
	if e.Deptid != 0 {
		table = table.Where("deptId = ?", e.Deptid)
	}
	if e.Deptname != "" {
		table = table.Where("dept_name = ?", e.Deptname)
	}

	if err := table.Where("is_del = 0").First(&doc).Error; err != nil {
		return doc, err
	}
	return doc, nil
}

func (e *Dept) GetDeptInfo() (*DeptInfo, error) {
	var info DeptInfo
	sql := `select d.*, u.nick_name leader_name from sys_dept as d left join sys_user as u on u.user_id = d.leader_id 
	where (d.deptId = ` + strconv.FormatInt(e.Deptid, 10) + `) and (d.is_del = 0)  
	order by d.deptId asc limit 1`

	if err := orm.Eloquent.Raw(sql).Scan(&info).Error; err != nil {
		return nil, err
	}
	return &info, nil
}

func (e *Dept) GetList() ([]Dept, error) {
	var doc []Dept

	table := orm.Eloquent.Table("sys_dept")
	if e.Deptid != 0 {
		table = table.Where("deptId = ?", e.Deptid)
	}
	if e.Deptname != "" {
		table = table.Where("dept_name = ?", e.Deptname)
	}
	if e.Status != "" {
		table = table.Where("status = ?", e.Status)
	}

	if err := table.Where("is_del = 0").Find(&doc).Error; err != nil {
		return doc, err
	}
	return doc, nil
}

func (e *Dept) GetPage(bl bool) ([]Dept, error) {
	var doc []Dept

	table := orm.Eloquent.Select("*").Table("sys_dept")
	if e.Deptid != 0 {
		table = table.Where("deptId = ?", e.Deptid)
	}
	if e.Deptname != "" {
		table = table.Where("dept_name = ?", e.Deptname)
	}
	if e.Status != "" {
		table = table.Where("status = ?", e.Status)
	}
	if e.DeptPath != "" {
		table = table.Where("deptPath like %?%", e.DeptPath)
	}
	//if e.TeamId != 0 {
	//	//	table = table.Where("team_id", e.TeamId)  //新加
	//	//}
	if bl {
		// 数据权限控制
		//dataPermission := new(DataPermission)
		////dataPermission.UserId, _ = utils.StringToInt64(e.DataScope)
		//table = dataPermission.GetDataScope("sys_dept", table)
	}

	if err := table.Where("is_del = 0").Find(&doc).Error; err != nil {
		return nil, err
	}
	return doc, nil
}

func (e *Dept) SetDept(bl bool) ([]Dept, error) {
	list, err := e.GetPage(bl)

	m := make([]Dept, 0)
	for i := 0; i < len(list); i++ {
		if list[i].ParentId != 0 {
			continue
		}
		info := Digui(&list, list[i])

		m = append(m, info)
	}
	return m, err
}

func Digui(deptlist *[]Dept, menu Dept) Dept {
	list := *deptlist

	min := make([]Dept, 0)
	for j := 0; j < len(list); j++ {

		if menu.Deptid != list[j].ParentId {
			continue
		}
		mi := Dept{}
		mi.Deptid = list[j].Deptid
		mi.ParentId = list[j].ParentId
		mi.DeptPath = list[j].DeptPath
		mi.Deptname = list[j].Deptname
		mi.Sort = list[j].Sort
		mi.LeaderID = list[j].LeaderID
		mi.Phone = list[j].Phone
		mi.Email = list[j].Email
		mi.Status = list[j].Status
		mi.CreateTime = list[j].CreateTime
		mi.UpdateTime = list[j].UpdateTime
		mi.IsDel = list[j].IsDel
		mi.Children = []Dept{}
		ms := Digui(deptlist, mi)
		min = append(min, ms)

	}
	menu.Children = min
	return menu
}

//func (e *Dept) Getdetpath(id int) (update Dept) {
//		adc:=id
//	sql := `select * from sys_dept where userid = ?`
//	orm.Eloquent.Raw(sql,adc).Scan(&update)
//	return
//}

func (e *Dept) Update(id int64) (update Dept, err error) {
	e.UpdateTime = time.Now()
	if err = orm.Eloquent.Table("sys_dept").Where("deptId = ?", id).First(&update).Error; err != nil {
		return
	}

	deptPath := "/" + utils.Int64ToString(e.Deptid)
	if int(e.ParentId) != 0 {
		var deptP Dept
		orm.Eloquent.Table("sys_dept").Where("deptId = ?", e.ParentId).First(&deptP)
		deptPath = deptP.DeptPath + deptPath
	} else {
		deptPath = "/0" + deptPath
	}
	e.DeptPath = deptPath

	//参数1:是要修改的数据
	//参数2:是修改的数据
	if err = orm.Eloquent.Table("sys_dept").Model(&update).Updates(&e).Error; err != nil {
		return
	}
	return
}

func (e *Dept) Delete(id int64) (success bool, err error) {
	var mp = map[string]string{}
	mp["is_del"] = "1"
	mp["update_time"] = time.Now().Format("2006/01/02 15:04:05")
	mp["update_by"] = e.UpdateBy
	if err = orm.Eloquent.Table("sys_dept").Where("deptId = ?", id).Update(mp).Error; err != nil {
		success = false
		return
	}
	success = true
	return
}

func (dept *Dept) SetDeptLable() (m []DeptLable, err error) {
	deptlist, err := dept.GetList()

	m = make([]DeptLable, 0)
	for i := 0; i < len(deptlist); i++ {
		if deptlist[i].ParentId != 0 {
			continue
		}
		e := DeptLable{}
		e.Id = deptlist[i].Deptid
		e.Label = deptlist[i].Deptname
		deptsInfo := DiguiDeptLable(&deptlist, e)

		m = append(m, deptsInfo)
	}
	return
}

func DiguiDeptLable(deptlist *[]Dept, dept DeptLable) DeptLable {
	list := *deptlist

	min := make([]DeptLable, 0)
	for j := 0; j < len(list); j++ {

		if dept.Id != list[j].ParentId {
			continue
		}
		mi := DeptLable{list[j].Deptid, list[j].Deptname, []DeptLable{}}
		ms := DiguiDeptLable(deptlist, mi)
		min = append(min, ms)

	}
	dept.Children = min
	return dept
}
