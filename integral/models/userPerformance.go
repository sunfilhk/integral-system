...
func (this *UserPerformance) GetTotal() (err error) {
	...
	sql1 = strings.Replace(sql1, ",,", ",", -1)

	var findTotal totalPerformance
	if err = orm.Eloquent.Raw(sql1).Scan(&findTotal).Error; err != nil {
		return
	}
	sql2 := `select nick_name,team_id  from sys_user where user_id=` + strconv.FormatInt(this.userID, 10)
	...
	this.DeptId = findName.TeamId
	this.NickName = findName.NickName
	return
}

func (this *UserPerformance) GetVipTotal() (err error) {
	sql := `select if(lifts<1,if(accumulative>set_level,if(level_lock=0,real_value+accumulative,GREATEST(set_level,lock_level)),GREATEST(set_level,lock_level)),GREATEST(set_level,lock_level)) accumulative from sys_user WHERE user_id = ` + strconv.FormatInt(this.userID, 10)
...
func (this *UserPerformance) GetVipLevel() (err error) {
	key := this.DeptId
	sql := `select * from user_level where team_id =?  `
	list := make([]UserLevelConfig, 0)
	if err = orm.Eloquent.Raw(sql, key).Scan(&list).Error; err != nil {
		return
	}
...
func (this *UserPerformance) GetMonth() (err error) {
	referrals, err1 := this.getUserReferrals()
	if err1 != nil {
		err = err1
		return
	...
	type totalMsgs struct {
		Total float64 `gorm:"column:total" json:"totals"`
	}
	var finds totalMsgs
	if err = orm.Eloquent.Raw(sql).Scan(&finds).Error; err != nil {
		return
	}
	fmt.Println("wew", finds.Total)
	this.Month = finds.Total
	return
}
