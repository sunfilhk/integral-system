...
func InitRouter() *gin.Engine {
	...
	//r.GET("/fill/lock_level", FillLockLevel) // 填充用户锁定的等级
	...
	auth.Use(authMiddleware.MiddlewareFunc()).Use(middleware.AuthCheckRole())
	{
		...
		//auth.GET("/getProfitcustomer", Profitcustomer)
		auth.GET("/getProfitcustomers", Profitcustomers)
		auth.GET("/getcustomerteam", Getcustomerteam)
		//auth.GET("/getProfitteams", GetProfitTeams)//首页获取下级的等级

		auth.GET("/customerList", GetcustomerList)           //客户列表
		auth.GET("/customerLists", CustomerLists)            //新增客户绑定investment
		auth.GET("/summrydetail", Summrydetail)              //业务员明细
		auth.GET("/searchsexpire", Searchs)                  //新模块搜索到期
		auth.POST("/customerAdd", CustomerAdd)               //添加客户
		auth.POST("/customerbili", GetKey)                   //获取客户分润比例
		...
		auth.GET("/investListss", InvestListss)                //新模块
		auth.POST("/investmentAdd", InvestmentAdd)             //添加客户投资
		auth.POST("/newinvestmentAdd", NewmoduleInvestmentAdd) //新模块添加客户投资
		...
		auth.GET("/getonceinto", Getonceinto)                        //新增一次性分配
		...
		auth.POST("/updateProfitcustomer", UpdateProfitscustomer)    //修改客户分润配置     新增
		...
		auth.GET("/teamList", GetTeamList)         // 获取团队类型列表
		auth.GET("/onecallocation", GetUserteamid) //一次性分配选择团队

		auth.GET("/searchpersonal", Searchpersonal) //新增查询个人业绩
...
func FillLockLevel(c *gin.Context) {
	once.Do(func() {
		// 获取用户列表信息
		var user models.SysUser
		var res models.Response
		userIds, err := user.GetUserIdList()
		if err != nil {
			res.Msg = "获取用户列表失败"
		...
		// 循环遍历所有用户,填充对应锁定的等级信息
		for _, val := range userIds {
			err := user.Updatelevellock(val)
			if err != nil {
				log.Println("订单", val, "填充用户锁定的等级失败")
				continue
			}
		}
		c.JSON(200, gin.H{
