    ...
    <el-dialog :title="title" :visible.sync="open" width="700px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="推荐人" prop="referrer_name">
              <el-input
                v-model="form.referrer_name"
                placeholder="请输入姓名、用户名、手机号查找"
                @input="chang"
              />
            </el-form-item>
          </el-col>
      ...
      <div v-show="show" class="small">
        <div v-for="(item, index) in arr" :key="index" @click="choice(item)">
    ...
    chang() {
      if (this.form.referrer_name.length != '') {
        userList({ keyword: this.form.referrer_name }).then(res => {
          this.arr = res.list
          this.team_id = res.list[0].team_id
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.userId !== undefined) {
            ...
            if (this.team_id) {
              if (this.form.deptId == this.team_id) {
                ...
                this.$confirm('推荐人与当前用户不在一个团队，请确认是否继续', {
                  confirmButtonText: '确定',
                  cancelButtonText: '取消',
                  type: 'info'
                }).then(async() => {
